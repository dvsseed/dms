...
class BasisT2dm1Transformer extends TransformerAbstract
{
    /**
    ...
    public function transform(BasisT2dm $basist2dm)
    {
        return [
            ...
            'symptom'           => json_decode($basist2dm->symptom),
            'treatment'         => json_decode($basist2dm->treatment),
            'medication'        => json_decode($basist2dm->medication),
            ...
            'comorbidity'       => json_decode($basist2dm->comorbidity),
            'lung'              => $basist2dm->lung,
            ...
            'complication'      => json_decode($basist2dm->complication),
            'heartdisease'      => json_decode($basist2dm->heartdisease),
            'blindness'         => $basist2dm->blindness,
            'dialysis'          => $basist2dm->dialysis,
            'amputation'        => $basist2dm->amputation,
            'fami_medic_his'    => json_decode($basist2dm->fami_medic_his),
            'relatives'         => json_decode($basist2dm->relatives),
            'hypertension'      => json_decode($basist2dm->hypertension),
            'cardiovascular'    => json_decode($basist2dm->cardiovascular),
            'stroke'            => json_decode($basist2dm->stroke),
            'activity'          => $basist2dm->activity,
            'education'         => $basist2dm->education,
            'occupation'        => $basist2dm->occupation,
            'worktime'          => $basist2dm->worktime,
            'fixedtime_from'    => json_decode($basist2dm->fixedtime_from),
            'fixedtime_to'      => json_decode($basist2dm->fixedtime_to),
            'dayshift_from'     => json_decode($basist2dm->dayshift_from),
            'dayshift_to'       => json_decode($basist2dm->dayshift_to),
            'middleshift_from'  => json_decode($basist2dm->middleshift_from),
            'middleshift_to'    => json_decode($basist2dm->middleshift_to),
            'nightshift_from'   => json_decode($basist2dm->nightshift_from),
            'nightshift_to'     => json_decode($basist2dm->nightshift_to),
            'affectlearning'    => json_decode($basist2dm->affectlearning),
